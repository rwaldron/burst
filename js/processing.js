...
this.Processing = function Processing( aElement, aCode) {
  ...
  if (aCode.substr(2,11) == 'burstEngine'){
      aCode = $.ajax({type: "GET", url: "../../../core/pde/burstCurrent.pde", async: false, cache: false}).responseText + aCode.split("//burstEngine")[1];
  } 
...
function buildProcessing( curElement ){        
  ...
  p.kill = function kill(mustdie){
    if(mustdie==curElement){
      p.isdead=true;
      window.Processing.data.kill=false;
    }
  }
  ...
  p.loadOff = function loadOff(loadUrl) {                 
      var points = $.ajax({type: "GET", url: loadUrl, async: false, cache: false}).responseText.split("\n");
      points.remove(0,1);
  ...
  // Added hexRGBA() - F1LT3R - 19.01.09
  p.hexRGBA = function hexRGBA( hex ){
  ...
  p.redraw = function redraw() {
    window.Processing.data.kill?p.kill(window.Processing.data.kill):0;
  ...
  p.loop = function loop() {
    ...
    looping = setInterval(function() {
      try {
        if (!p.paused && !p.hasDelay && !p.isdead){
          p.redraw();  
        }      
      }
      catch(e) {
...
// Define sharring area for canvases
